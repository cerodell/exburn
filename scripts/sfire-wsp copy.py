...
ax.plot(met_df.index, met_df[" WS_ms"], color="tab:blue", lw=1.5, linestyle="--")
...
colors_default = iter(cm.Dark2(np.linspace(0, 1, len(hfx_dfs) + 4)))

hfx_list = hfx_ds.hfxs.values
for i in range(len(hfx_dfs)):
    c = next(colors_default)
    ax.plot(hfx_dfs[i].index, np.abs(hfx_dfs[i]["HFX"]), color=c, linestyle="--")
    ax.plot(
...
cbar.set_label("Wind Speed \n (m s^-1)", rotation=90, fontsize=10, labelpad=15)
shape = XLAT.shape
dxy = 25
