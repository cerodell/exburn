...
wrf_ds_int = xr.open_dataset(str(data_dir) + f"/{modelrun}/wspd_wdir_intial.nc")
wrf_ds_int = wrf_ds_int.sel(
...
wrf_ds["wsp_anomaly"] = wrf_ds["wspd"] - wrf_ds_int["wspd"]
...
fig.suptitle(
    f"Vertical velocity and Horizontal wind streamlines \n for varied heights above ground level (AGL) at {str(times)[:-10]} \n \n {modelrun} \n \n"
)
...
for i in range(len(height[: (col * row)])):
    ...
    wrf_dsi = wrf_ds.isel(bottom_top=i)
    contour = ax.contourf(
        XLONG,
        XLAT,
        wrf_dsi["wsp_anomaly"],
...
cbar.set_label("Relative Horizontal Velocity (m/s)", fontsize=10)
# plt.tight_layout(pad=0.2, w_pad=0.2, h_pad=0.2)
plt.savefig(str(save_dir) + f"/Relative-Horizontal-Velocity.png", dpi=250)
