...
fulltime = (model_time[-1] - model_time[0]).astype("timedelta64[s]")
max_ind = ros_da.argmax(dim="Time").values
all_offset = []
for i in range(len(ros_dfs)):
    obs_time = ros_dfs[i]["temp"].idxmax()
    off_set = (model_time[max_ind[i]] - obs_time).total_seconds()
    all_offset.append(off_set)
ros_ids = np.array(ros_ids)
all_offset = np.array(all_offset)
index_by_ros = [3, 4, 5, 6, 7, 8, 14, 15, 16, 17, 18, 19, 20, 21, 22]
print(ros_ids[index_by_ros])
print(np.mean(all_offset[index_by_ros]))
print((np.mean(all_offset[index_by_ros]) / 499) * 100)


fig = plt.figure(figsize=(12, 4))
fig.suptitle("Offset of Fire Front Arrival Time \n Model minus Observed ")
ax = fig.add_subplot(1, 1, 1)
ax.bar(ros_ids, all_offset)
